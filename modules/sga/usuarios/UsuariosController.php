...
class UsuariosController extends CrudController
{
    ...
    protected function preSave(Context $context, SequencialModel $model)
    {
        ...
        if (empty($lotacoes)) {
            throw new Exception(_('El usuario debe poseer al menos una situación.'));
        }
        if ($model->getId() == 0) {
        ...
        if ($rs['total']) {
            throw new \Exception(_('El login solicitado ya está asignado a otro usuario.'));
        }
    }
    ...
    protected function preDelete(Context $context, SequencialModel $model)
    {
        if ($context->getUser()->getId() === $model->getId()) {
            throw new \Exception(_('No es posible eliminarse a si mismo.'));
        }
        ...
        if ($total > 0) {
            throw new \Exception(_('No es posible eliminar ese usuario pues el mismo ya realizó atenciones.'));
        }
    ...
    public function alterar_senha(Context $context)
    {
        ...
        if ($usuario) {
            ...
            $response->message = _('Usuario inválido');
        }
