...
class GruposController extends TreeModelController
{
    ...
    protected function preSave(Context $context, SequencialModel $model)
    {
        ...
        if ($model->getId() == 0) {
            if (!$pai) {
                throw new \Exception(_('Por favor escoger el Grupo padre'));
            }
    ...
    protected function preDelete(Context $context, SequencialModel $model)
    {
        ...
        $rs = $query->getSingleResult();
        if ($rs['total'] > 0) {
            throw new \Exception(_('Ese grupo no puede ser eliminado porque posee relacionamiento con una o más unidades.'));
        }
    }
}
