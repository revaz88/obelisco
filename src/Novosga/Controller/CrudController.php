            ...
            try {
                foreach ($requiredFields as $field) {
                    ...
                    if (empty($value) && $value !== '0') {
                        throw new \Exception(_('Llenar los campos obligatorios'));
                    }
                ...
                if ($id > 0) { // editando
                    ...
                    $message['text'] = _('Registro modificado exitosamente');
                } else { // criando
                    $this->doSave($context, $this->model);
                    $id = $this->model->getId();
                    $redirUrl .= '/'.$id;
                    if ($id > 0) {
                        $message['text'] = _('Nuevo registro agregado exitosamente');
                    } else {
                        $message['text'] = _('Error al guardar el nuevo registro. Por favor intente nuevamente.');
    ...
    public function delete(Context $context, $id)
    {
        ...
        if ($model) {
            try {
                ...
                $this->app()->flash('success', _('Registro eliminado exitosamente'));
            } catch (Exception $e) {
                try {
