...
abstract class TreeModelController extends CrudController
{
    ...
    protected function doSave(Context $context, SequencialModel $model)
    {
        if (!($model instanceof TreeModel)) {
            throw new Exception(sprintf(_('Modelo inválido pasado como parámetro. Era esperado TreeModel y pasó %s'), get_class($model)));
        }
    ...
    private function persist(TreeModel $model)
    {
        ...
        try {
            ...
            $this->em()->rollback();
            throw new Exception(sprintf(_('Error al insertar el registro: %s'), $e->getMessage()));
        }
    }
    ...
    private function merge(TreeModel $model)
    {
        try {
            ...
            $this->em()->rollback();
            throw new Exception(sprintf(_('Error al actualizar el registro: %s'), $e->getMessage()));
        }
    }
    ...
    {
        if ($model->getLeft() == 1) {
            throw new Exception(_('No puede eliminar la raiz'));
        }
        try {
            $this->em()->beginTransaction();
            ...
            $this->em()->rollback();
            throw new Exception(sprintf(_('Error al borrar el registro: %s'), $e->getMessage()));
        }
