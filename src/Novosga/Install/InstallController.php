...
class InstallController extends InternalController
{
    ...
    {
        if (empty(self::$steps)) {
            self::$steps[] = new InstallStep(0, _('Inicio')); // install welcome
            self::$steps[] = new InstallStep(1, _('Verificación de Requisitos')); // install check
            self::$steps[] = new InstallStep(2, _('Licencia')); // license
            self::$steps[] = new InstallStep(3, _('Configurar Base de datos')); // DB
    ...
    public function step1(Context $context, array &$data)
    {
        ...
        $requiredsPermission = array(
            array('label' => _('Configuración de saturno'), 'file' => NOVOSGA_CONFIG, 'required' => _('Escrita')),
            array('label' => _('Directorio temporal'), 'file' => sys_get_temp_dir(), 'required' => _('Escrita')),
        ...
        foreach ($requiredsPermission as &$req) {
            if (is_writable($req['file'])) {
                ...
                $fatal = true;
                $req['result'] = _('No existe');
                $req['class'] = 'danger';
            } else {
                $fatal = true;
                $req['result'] = _('Solo lectura');
                $req['class'] = 'danger';
            }
        }
    ...
    public function set_adapter(Context $context)
    {
        ...
        if ($context->request()->isPost()) {
            ...
            if (array_key_exists($adapter, InstallData::$dbTypes)) {
                ...
                $response->message = sprintf(_('Opción inválida: %s'), $adapter);
            }
        } else {
    ...
    public function info(Context $context)
    {
        if (!App::isInstalled()) {
            ...
            echo _('Por razones de seguridad las informaciones sobre el entorno son deshabilitadas después de la instalación');
        }
        exit();
    }
    ...
    {
        if ($context->request()->isPost()) {
            $response = new JsonResponse(true, _('Base de Datos comprobada exitosamente!'));
            $session = $context->session();
            ...
            try {
                ...
                if (!file_exists($sqlFile)) {
                    throw new Exception(_('No se encontró el archivo SQL para el tipo de base escogido'));
                }
    ...
    {
        if ($context->request()->isPost()) {
            $response = new JsonResponse(true, _('Datos de usuario informados exitosamente'));
            $session = $context->session();
            $data = $session->get(InstallData::SESSION_KEY);
            if (!$data) {
            ...
            try {
                ...
                if (!ctype_alnum($adm['login'])) {
                    throw new Exception(_('El login debe contener solo letras y números'));
                }
                if (strlen($adm['login']) < 5 || strlen($adm['login']) > 20) {
                    throw new Exception(_('El login debe poseer entre 5 y 20 caracteres (letras o números)'));
                }
                if (strlen($adm['senha']) < 6) {
                    throw new Exception(_('La contraseña debe poseer 6 o más letras/números'));
                }
                if ($adm['senha'] != $context->request()->post('senha_2')) {
                    throw new Exception(_('La contraseña no coincide con la confirmacion de contraseña'));
                }
                $data->admin = $adm;
            } catch (Exception $e) {
                $response->success = false;
    ...
    {
        if ($context->request()->isPost()) {
            $response = new JsonResponse(true, _('Instalación terminada con éxito'));
            $conn = null;
            $session = $context->session();
            try {
                if (App::isInstalled()) {
                    throw new Exception(_('El saturno ya está instalado'));
                }
                $data = $session->get(InstallData::SESSION_KEY);
                if (!$data) {
                    throw new Exception(_('No se encontraron datos de instalación. Por favor inicie de nuevo'));
                }
                ...
                if ($version) {
                    ...
                    foreach ($scripts as $sql) {
                        if (!is_readable($sql)) {
                            $msg = _('Script SQL de instalación no encontrado (%s)');
    ...
    {
        return new JsonResponse(false, _('Solicitud inválida'));
    }
    ...
    public static function migrationScripts($from, $to)
    {
        ...
        if (!is_dir($path)) {
            throw new Exception(sprintf('Ruta a nueva versión no válida: %s', $path));
        }
        $scripts = array();
        $files = glob("$path/*.sql");
        foreach ($files as $file) {
